...
#define muon_printf(...)                            \
    ...
    */
    #define muon_type_printer(...) \
        ...
        muon_printf("If you think this was an error, please file an issue on Muon' Github repo.")
...
#define CHECK_TRUE(x)                                                         \
    do {                                            \
        if (!(x)) {                                                                \
            muon_printf("%s:%u: Failure\n", __FILE__, __LINE__);                    \
            muon_printf("  Expected : MUON_true\n");                                     \
            muon_printf("    Actual : %s\n", (x) ? "MUON_true" : "MUON_false");               \
            *muon_result = 1;                                                       \
        }                                                                          \
    }                                                                            \
    while (0)                                                                    
...
/*
   define a main() function to call into muon.h and start executing tests! A
   user can optionally not use this macro, and instead define their own main()
   function and manually call muon_main. The user must, in exactly one source
   file, use the MUON_STATE macro to declare a global struct variable that
   muon requires.
*/
#define MUON_MAIN()     \
    /* we need, in exactly one source file, define the global struct that will hold */ \
    /* the data we need to run muon. This macro allows the user to declare the */ \
    /* data without having to use the MUON_MAIN macro, thus allowing them to write */ \
